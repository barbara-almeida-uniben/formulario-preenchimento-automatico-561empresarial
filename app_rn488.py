...
st.write("RN488 padrão / Sul Goiás + Termo de Responsabilidade Financeira")
...
def gerar_pdf_rn488(codigo, registro_produto, data_rescisao, motivo_desligamento):
    ...
    if erro_unimed:
        return None, None, erro_unimed

    nome_operadora_detectada = str(row["OPERADORA"]).strip().upper()

    if "SUL GOIAS" in nome_operadora_detectada or "SUL GOIÁS" in nome_operadora_detectada:
        arquivo_modelo = "rn488_sulgoias.pdf"
        tipo_formulario = "SUL_GOIAS"
    else:
        arquivo_modelo = "formulario_rn488.pdf"
        tipo_formulario = "PADRAO"

    pdf = PdfReader(arquivo_modelo)
    ...
    if tipo_formulario == "SUL_GOIAS":
        campos_pdf["Texto8"] = data_exclusao
    else:
        campos_pdf["Texto4"] = data_exclusao

    motivo = str(motivo_desligamento).strip().upper()

    if tipo_formulario == "SUL_GOIAS":
        mapa_motivo_checkbox = {
            "SEM JUSTA CAUSA": "CHEK0114",
            "COM JUSTA CAUSA": "CHEK0112",
            "PEDIDO DE DEMISSAO": "CHEK0110",
            "PEDIDO DE DEMISSÃO": "CHEK0110",
        }
    else:
        mapa_motivo_checkbox = {
    ...
    prefixo = "RN488_SulGoias" if tipo_formulario == "SUL_GOIAS" else "RN488"

    nome_pdf = f"{prefixo}_{codigo}_{nome_seguro(row['NOME'])}.pdf"
    ...
    for page in pdf.pages:
        ...
        for annotation in annotations:
            ...
            if campo in campos_termo:
                annotation.update(
                    PdfDict(V=str(campos_termo[campo]))
                )

    nome_pdf = f"Termo_Resp_Financeira_{codigo}_{nome_seguro(row['NOME'])}.pdf"
...
if modo == "Individual":
    ...
    data_rescisao = st.text_input("Digite a data da rescisão - DD/MM/AA")
    ...
    if st.button("Gerar documentos"):
        ...
        if not codigo or not registro_produto or not data_rescisao or not motivo_desligamento:
    ...
    st.write(
        "A aba deve conter: CODIGO, REGISTRO_PRODUTO, DATA_RESCISAO e MOTIVO_DESLIGAMENTO."
    )
    ...
    if st.button("Gerar documentos em lote"):
        ...
        for _, item in lote.iterrows():
            ...
            caminho_rn488 = os.path.join(pasta_saida, arquivo_rn488)
            caminho_termo = os.path.join(pasta_saida, arquivo_termo)
            ...
            os.replace(arquivo_rn488, caminho_rn488)
            os.replace(arquivo_termo, caminho_termo)

            arquivos_gerados.append(caminho_rn488)
            arquivos_gerados.append(caminho_termo)
