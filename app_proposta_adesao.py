...
if st.button("Gerar proposta"):
    ...
    if not codigo or not data_vigencia or not produto_escolhido:
    ...
    if tipo_processo == "Inclusão de dependente":
        ...
        if not cpf_novo_dependente:
            st.error("Informe o CPF do novo dependente.")
            st.stop()
        ...
        if dependentes.empty:
            st.error("Dependente não encontrado pelo CPF informado.")
            st.stop()
    ...
    produto_base = produtos[
        produtos["Produto"].str.strip().str.upper() == produto_escolhido.strip().upper()
    ]
    ...
    # =====================================================
    # ENDEREÇO E VALOR TOTAL POR COORDENADAS
    # =====================================================
    ...
    can.drawString(500, 215, str(row["UF"]))  

    # valor total na página 4
    can.showPage()
    can.showPage()
    can.showPage()
    ...
    with open(nome_pdf_final, "rb") as file:
        st.download_button(
            label="⬇️ Baixar proposta",
            data=file,
            file_name=nome_pdf_final,
